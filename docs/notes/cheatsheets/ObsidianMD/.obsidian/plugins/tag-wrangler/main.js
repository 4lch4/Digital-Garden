...
var fullstore = value => {
  ...
  return (...args) => {
    ...
    if (!args.length) return data.value
...
const setAttribute = currify_1((el, obj, name) =>
...
const query$1 = currify_1((element, name) =>
  element.querySelector(`[data-name="js-${name}"]`)
)

function find(element, names) {
  const elements = names.map(query$1(element)).filter(Boolean)
...
class Progress {
  ...
  set title(text) {
    this.dialog.querySelector('header').textContent = text
  }
  get title() {
...
async function validatedInput(
  ...
  while (true) {
    ...
    inputField.oninput = () =>
      inputField.setAttribute('aria-invalid', !isValid(inputField.value))
    ...
    for (let ch = src[offset]; ch === '\n'; ch = src[offset]) {
      ...
      if (!this.blockIndent) {
        ...
        if (!this.error) {
          ...
          const msg =
            'Block scalars must not be less indented than their '.concat(src)
          this.error = new YAMLSemanticError(this, msg)
        }
      }
...
class PlainValue extends Node {
  ...
  get strValue() {
    ...
    switch (ch0) {
      ...
      case '@':
      case '`': {
        const msg = 'Plain value cannot start with reserved character '.concat(
          ch0
        )
        const errors = [new YAMLSemanticError(this, msg)]
        return {
  ...
  parseCharCode(offset, length, errors) {
    ...
    if (isNaN(code)) {
      errors.push(
        new YAMLSyntaxError(
          this,
          'Invalid escape sequence '.concat(src.substr(offset - 2, length + 2))
        )
      )
...
class Collection$1 extends Node$1 {
  ...
  getIn([key, ...rest], keepScalar) {
    ...
    if (rest.length === 0)
      ...
      return node instanceof Collection$1
    ...
    if (nodes.length === 0) {
      ...
      if (
        ...
        strings.reduce((sum, str) => sum + str.length + 2, 2) >
          Collection$1.maxFlowStringSingleLineLength
      ) {
        str = start
  ...
  if (key instanceof Node$1 && ctx && ctx.doc) {
    ...
    if (!ctx.mapKeyWarned) {
      ...
      warn(
        ...
        'Keys with collection values will be stringified due to JS Object restrictions: '.concat(
          jsonStr,
          '. Set mapAsMap: true to use object keys.'
        )
      )
...
class Pair extends Node$1 {
  ...
  toString(ctx, onComment, onChompKeep) {
    ...
    let explicitKey =
      ...
      (!key ||
        keyComment ||
        (key instanceof Node$1
          ? key instanceof Collection$1 ||
    ...
    if (value instanceof Node$1) {
      ...
      if (value.commentBefore) {
        const cs = value.commentBefore.replace(
          /^/gm,
          ''.concat(ctx.indent, '#')
        )
        vcb += '\n'.concat(cs)
...
function foldFlowLines(
  ...
  for (let i = 0; i < folds.length; ++i) {
    ...
    if (fold === 0) res = '\n'.concat(indent).concat(text.slice(0, end))
    else {
  ...
  for (const { key } of pairs.items) {
    if (key instanceof Scalar) {
      if (seenKeys.includes(key.value)) {
        onError(
          'Ordered maps must not include duplicate keys: '.concat(key.value)
        )
      } else {
  ...
  [
    {
      ...
      options: nullOptions,
      stringify: ({ sourceStr }) =>
        sourceStr !== null && sourceStr !== void 0
          ? sourceStr
          : nullOptions.nullStr
    },
    trueObj,
    ...
    if (typeof tag === 'string') {
      ...
      if (!tagObj) {
        const keys = Object.keys(knownTags)
          ...
          .join(', ')
        throw new Error(
          'Unknown custom tag "'.concat(tag, '"; use one of ').concat(keys)
        )
      }
...
function resolveTagName(doc, node) {
  ...
  if (tag) {
    ...
    if (verbatim) {
      ...
      const msg = "Verbatim tags aren't resolved, so ".concat(
        verbatim,
        ' is invalid.'
      )
    ...
    if (tag.tag === tagName) {
      if (tag.test) {
        if (typeof value === 'string') matchWithTest.push(tag)
          ...
          onError(
            'The tag '.concat(tagName, ' cannot be applied to a collection')
          )
      } else {
    ...
    switch (node.type) {
      ...
      case Type.MAP:
        ...
        if (tagName === SEQ || tagName === STR)
          onError('The tag '.concat(tagName, ' cannot be applied to a mapping'))
        break
      ...
      case Type.SEQ:
        ...
        if (tagName === MAP || tagName === STR)
          onError(
            'The tag '.concat(tagName, ' cannot be applied to a sequence')
          )
        break
      ...
      default:
        ...
        if (tagName === MAP || tagName === SEQ)
          onError('The tag '.concat(tagName, ' cannot be applied to a scalar'))
        fallback = STR
    }
...
function parseContents(doc, contents) {
  ...
  if (!body) {
    ...
    if (cb) {
      const cbNode =
        body instanceof Collection$1 && body.items[0] ? body.items[0] : body
...
function resolveYamlDirective(doc, directive) {
  ...
  if (!documentOptions[version]) {
    ...
    const msg = 'Document will be parsed as YAML '
...
class Document$1 {
  ...
  getDefaults() {
    return (
      Document$1.defaults[this.version] ||
      Document$1.defaults[this.options.version] ||
      {}
    )
  }

  get(key, keepScalar) {
    return this.contents instanceof Collection$1
...
class File {
  ...
  async renamed(replace) {
    ...
    if (this.hasFrontMatter) text = this.replaceInFrontMatter(text, replace)
  ...
  replaceInFrontMatter(text, replace) {
    ...
    if (parsed.errors.length) {
      ...
      console.error(error)
      new obsidian.Notice(error + '; skipping frontmatter')
      return
    }
    ...
    for (const {
      ...
      if (typeof field === 'string') {
        ...
        if (field != after) {
          parsed.set(prop, after)
          changed = true
        }
      } else if (Array.isArray(field)) {
        replace.inArray(field).forEach((v, i) => {
          if (field[i] !== v) node.set(i, v)
          changed = true
        })
      }
    }
...
async function renameTag(app, tagName) {
  ...
  const oldTag = new Tag(tagName),
    ...
    shouldAbort =
      clashing && (await shouldAbortDueToClash(clashing, oldTag, newTag))
  ...
  return new obsidian.Notice(
    `Operation ${
      progress.aborted ? 'cancelled' : 'complete'
    }: ${renamed} file(s) updated`
  )
}
...
async function findTargets(app, tag) {
  ...
  await progress.forEach(app.metadataCache.getCachedFiles(), filename => {
    ...
    const fmtags = (obsidian.parseFrontMatterTags(frontmatter) || []).filter(
      tag.matches
    )
...
async function promptForNewName(tagName) {
  try {
    return await validatedInput(
      `Renaming #${tagName} (and any sub-tags)`,
...
class TagWrangler extends obsidian.Plugin {
  ...
  onMenu(e, tagEl) {
    ...
    const tagName = tagEl.find('.tag-pane-tag-text').textContent,
    ...
    if (search) {
      ...
      if (query) {
        menu.addItem(
          item('sheets-in-box', 'Require #' + tagName + ' in search', () =>
            search.openGlobalSearch(query + ' tag:' + tagName)
          )
        )
      }
      menu.addItem(
        item('crossed-star', 'Exclude #' + tagName + ' from search', () =>
          search.openGlobalSearch(query + ' -tag:' + tagName)
        )
      )
    }
    ...
    if (isHierarchy) {
      ...
      menu
        .addSeparator()
        .addItem(
          item('vertical-three-dots', 'Collapse tags at this level', () =>
            toggle(true)
          )
        )
        .addItem(
